...
export async function POST(req: NextRequest) {
  ...
  if (file.size > maxBytes) {
    return jsonError({
      code: ErrorCode.PdfTooLarge,
      message: `File size exceeds the ${env.maxPdfMb()} MB limit.`,
      status: 413,
      headers: NO_STORE_HEADERS,
    });
  }
  ...
  try {
    ...
    return jsonError({
      code: ErrorCode.PdfExtractionFailed,
      message: 'Failed to extract text from the PDF. Ensure the file is not encrypted or image-only.',
      status: 422,
      headers: NO_STORE_HEADERS,
    });
  }
  ...
  if (!text || text.trim().length < minChars) {
    return jsonError({
      code: ErrorCode.ResumeTooShort,
      message: `Extracted text is too short. At least ${minChars} characters are required.`,
      status: 422,
      headers: NO_STORE_HEADERS,
    });
  }
